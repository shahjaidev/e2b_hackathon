...
interface CodeMessagePreviewProps {
    message: {
        id: string
        fragmentId?: string
        code?: string
...
const CodeMessagePreview = ({ message, fragment, toggleCode }: CodeMessagePreviewProps) => {
    const { code } = useCode()
    // Use message.code if available, otherwise try to get from context
    const codeContent = message.code || (message.fragmentId ? code[message.fragmentId] : '') || ''
